...
- 读取本机 WorkBuddy 生命周期日志，并用不同应用 Logo 与底色区分任务来源
- 多任务常驻摘要及展开查看
- 多 Agent 子任务数量、层级展开、当前工具动作与 Queue 排队数量
- 识别 Codex Voice 语音会话，开始时显示语音状态，结束后自动清除
...
- Codex 与 WorkBuddy 任务统一显示应用 Logo、状态点和来源底色，点击任务打开对应应用。
- 设置页增加监测状态与本地诊断复制入口。
- 补充 Codex 桌面端转发事件兼容，减少部分任务无法及时显示的问题。

### v0.1.6 · Voice 状态与额度细节

- 增加 Codex Voice 会话状态，语音开始和结束均以真实桌面事件为准。
- 修复 Codex 复用历史语音任务时，状态无法出现或关闭后仍然残留的问题。
- 剩余额度低于 1% 时按 0% 展示，并在额度用尽时显示预计重置时间。
- 移除日报彩带动画，保持统一、轻量的刘海动效。

### v0.1.5 · Agent 运行现场

- 展示真实子 Agent 数量，并可展开查看父任务与子任务层级。
- 展示当前工具图标和动作名称，以及后续追加消息的 Queue 数量。
- 保持物理摄像头区域完全留空，新增信息只进入两侧安全区和任务详情行。
