...
可以从 [GitHub Releases](https://github.com/49LabsDev/CodexNotch/releases/latest) 下载 Apple 芯片版 DMG，将应用拖入“应用程序”文件夹；也可以从源码构建：

```sh
git clone https://github.com/49LabsDev/CodexNotch.git
cd CodexNotch
sh build-app.sh
...
- 自动读取本机 Codex 会话记录。
- 展示运行、分析、等待确认、完成和异常。
- 展示任务名称、最近结果摘要、模型、推理强度、单任务 Token 和运行时间。
...
- 空闲悬停显示今日 Token 增量、重置倒计时、方案类型和 Codex 版本。
